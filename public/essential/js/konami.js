		...
		audio.onended = function () {
			...
			if ($(window).width() < 800) {
				Array.from(document.querySelectorAll('body')).forEach((el) =>
					el.classList.add('konamim')
				);
				Array.from(document.querySelectorAll('body')).forEach((el) =>
					el.classList.remove('konami')
				);
			} else {
				Array.from(document.querySelectorAll('body')).forEach((el) =>
					el.classList.add('konami')
				);
				Array.from(document.querySelectorAll('body')).forEach((el) =>
					el.classList.remove('konamim')
				);
			}
			Array.from(document.querySelectorAll('body')).forEach((el) =>
				el.classList.remove('darling', 'darlingm', 'monika')
			);
			Array.from(document.querySelectorAll('body')).forEach((el) =>
				el.classList.remove('monika')
			);
