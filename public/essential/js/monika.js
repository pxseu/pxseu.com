...
function monikaCodePassed() {
	if ($("body").hasClass("monika")) {
		...
		Array.from(document.querySelectorAll("body")).forEach((el) =>
			el.classList.remove("darling", "konami", "darlingm", "konamim")
		);
		Array.from(document.querySelectorAll(".box")).forEach((el) =>
			el.classList.remove("box-show")
		);
		Array.from(document.querySelectorAll("ul")).forEach((el) =>
			el.classList.remove("show")
		);
		Array.from(document.querySelectorAll("body")).forEach((el) =>
			el.classList.add("videoplay")
		);
		...
		monikaAudio.play();
		Array.from(
			document.querySelectorAll(".btn , .menubtn , button , .container, .clock")
		).forEach((el) => el.classList.add("hideall", "slowmoshow"));
		$("#gifbox").remove();
		...
		document.getElementById("video").onended = function () {
			...
			Array.from(document.querySelectorAll(".button1")).forEach((el) =>
				el.classList.add("monikabtn")
			);
			Array.from(
				document.querySelectorAll(
					".btn , .menubtn , .apply , .container , .clock"
				)
			).forEach((el) => el.classList.remove("hideall"));
			$("#video").remove();
			...
			setTimeout(function () {
				Array.from(
					document.querySelectorAll(
						".btn, .menubtn, .button1, .container, .clock"
					)
				).forEach((el) => el.classList.remove("slowmoshow"));
			}, 2300);
		};
	}
}
function pinker2() {
	Array.from(document.querySelectorAll("body")).forEach((el) =>
		el.classList.remove("darling", "konami")
	);
	Array.from(document.querySelectorAll("body")).forEach((el) =>
		el.classList.add("monika")
	);
	Array.from(document.querySelectorAll(".text1")).forEach((el) =>
		el.classList.add("transparent")
	);
	Array.from(
		document.querySelectorAll(
			".text2, .contactStuff, h1 strong, button, .btn, .menubtn, li a, .box h2, .apply, .clock, .box, .playlist, ul"
		)
	).forEach((el) => el.classList.add("pink"));
	document
		.getElementById("clickmebtn")
		.setAttribute("onClick", "monikamusicstop()");
}
...
function monikamusicstop() {
	...
	document
		.getElementById("clickmebtn")
		.setAttribute("onClick", "monikamusicplay()");
}
