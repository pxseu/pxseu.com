...
import { REDIS_LAST_UPDATE_ON, type default as SpotifyClient } from "../clients/spotify.js";
...
export class SpotifyRealtimeClient extends RealtimeClient<typeof REDIS_SPOTIFY_PLAYING, Song> {
	async initialize() {
		...
		let currentPlaying: Song = await this.redis.get(REDIS_SPOTIFY_PLAYING).then((v) => JSON.parse(v || "null"));
		let lastUpdateOn = await this.redis.get(REDIS_LAST_UPDATE_ON).then((v) => parseInt(v || "0", 10));

		await publisher.subscribe(REDIS_SPOTIFY_PLAYING, async (message, channel) => {
